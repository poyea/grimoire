...
$A = U Sigma V^top$ exists for any matrix and is the eigendecomposition done right for nonsymmetric and rectangular data: singular values are the square roots of the eigenvalues of $A^top A$, but forming $A^top A$ explicitly squares the condition number and destroys small singular values below $sqrt(u) parallel A parallel$ (an eigenvalue $sigma_i^2$ is lost once it falls below $u parallel A parallel^2$). The Golub-Kahan algorithm avoids this: bidiagonalize $A$ by Householder reflectors from both sides, then apply implicit-shift QR to the bidiagonal matrix; total $O(m n^2)$, backward stable. Variants: divide-and-conquer (`gesdd`, faster for full SVD, the `numpy.linalg.svd` default) and Jacobi SVD (slowest, but computes small singular values to high *relative* accuracy).
