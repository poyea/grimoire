...
*See also:* _Simply-Typed Lambda Calculus_, #xref("programming-languages", "type-systems", label: "Type Systems"), _Dependent Types_, #xref("programming-languages", "categorical-semantics", label: "Categorical Semantics")
...
satisfies $Y space f arrow.r^* f (Y space f)$ and produces non-terminating reductions. If $Y$ were typable in System F at the polymorphic type $tau = forall alpha . (alpha arrow.r alpha) arrow.r alpha$, then for any type $alpha$ and any $f : alpha arrow.r alpha$, the term $Y space [alpha] space f$ would be a closed well-typed term of type $alpha$. By Strong Normalization, $Y space f$ is SN. But $Y space f$ reduces to $f (Y space f)$ which reduces to $f (f (Y space f))$ etc., yielding infinite reduction: a contradiction. Hence no fixed-point combinator is typable in System F. Adding $"fix"$ as a primitive (with its own typing rule $forall alpha . (alpha arrow.r alpha) arrow.r alpha$ and reduction rule) makes the system Turing-complete but destroys SN and logical consistency.
...
Types of kind $*$ are *proper* types (inhabited by terms). Types of higher kinds are *type operators*, i.e., type-level functions.

*Examples.*
- $"List" : * arrow.r *$: a type operator taking a type to a type.
- $"State" : * arrow.r * arrow.r *$: state transformer.
- $"Functor" : (* arrow.r *) arrow.r *$? In Haskell `Functor` is a type *class*, but the underlying constructor maps a functor (kind $* arrow.r *$) to a constraint (kind `Constraint`).
