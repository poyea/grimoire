...
#theorem(name: "Girard 1972, Coquand 1986")[The system *MLTT + $cal(U) : cal(U)$* (i.e., a single universe containing itself) is inconsistent.]
...
#theorem(name: "Martin-Löf")[W-types together with $Pi, Sigma$, unit, empty, sums, and identity give a system in which every strictly-positive inductive type is definable.]
...
#theorem(name: "Hofmann–Streicher 1998")[There is a model of ITT in which types are *groupoids* (categories where every morphism is invertible), terms are objects, and propositional equality is *isomorphism*. In this model, UIP fails: two different isomorphisms can yield two different "proofs" of an equality.]
