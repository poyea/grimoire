...
*See also:* #xref("programming-languages", "semantics", label: "Program Semantics"), #xref("programming-languages", "type-systems", label: "Type Systems")
...
CCS is the prototypical synchronous, name-binding process calculus. Names are atomic; they have no internal structure and cannot be transmitted. Communication is *handshake* synchronisation: a sender on channel $a$ and a receiver on $overbar(a)$ rendezvous and produce a single internal action $tau$.

=== Syntax

Fix a countable set $cal(N)$ of *names*, ranged over by $a, b, c, dots$. The *co-names* are $overbar(cal(N)) = { overbar(a) : a in cal(N) }$, with $overbar(overbar(a)) = a$. The *action set* is

$ "Act" = cal(N) union overbar(cal(N)) union {tau} $

ranged over by $alpha$. Visible actions are elements of $cal(N) union overbar(cal(N))$; $tau$ is the *silent* or *internal* action. Process identifiers are drawn from a separate set; each identifier $A$ has a defining equation $A eq.delta P$.
...
The COM rule is the heart of CCS: a send on $a$ and a receive on $overbar(a)$ in parallel synchronise and yield a $tau$. Note that this is *binary* synchronisation; CCS provides no primitive for multi-way rendezvous (CSP does, via its synchronisation set). Restriction $(nu a)$ blocks any visible action on $a$ from being observable from outside, but does not forbid internal synchronisations on $a$ between subprocesses.

=== Example: A Two-Place Buffer

A one-place cell that can be filled on channel $"in"$ and emptied on $overbar("out")$:

$ "Cell" eq.delta "in".overbar("out").italic("Cell") $
...
$ P, Q ::= 0 | overbar(x) chevron.l y chevron.r. P | x(y). P | P bar Q | (nu x) P | !P | P + Q $

- $overbar(x) chevron.l y chevron.r. P$: send name $y$ on channel $x$, then continue as $P$.
...
$ alpha ::= tau | overbar(x) chevron.l y chevron.r | x(y) | overbar(x)(y) $

with $overbar(x)(y)$ the *bound output* indicating the extrusion of a private name $y$ on $x$. The rules include the analogues of CCS's ACT, PAR, COM, RES, SUM, with an *OPEN* rule
...
$ P ::= dots | overbar(x) chevron.l P chevron.r. Q | x(X). Q | X $

Sangiorgi (1992) showed that HO$pi$ has no extra expressive power: there is a faithful translation

$ [| overbar(x) chevron.l R chevron.r. P |] = (nu r)(overbar(x) chevron.l r chevron.r. ([| P |] bar !r(y). [| R |])) $
...
$ chevron.l chevron.l x chevron.r chevron.r_v p &= overbar(p) chevron.l x chevron.r \
chevron.l chevron.l lambda x. M chevron.r chevron.r_v p &= (nu f)(overbar(p) chevron.l f chevron.r. !f(x, q). chevron.l chevron.l M chevron.r chevron.r_v q) \
chevron.l chevron.l M N chevron.r chevron.r_v p &= (nu q)(chevron.l chevron.l M chevron.r chevron.r_v q bar q(f). (nu r)(chevron.l chevron.l N chevron.r chevron.r_v r bar r(v). overbar(f) chevron.l v, p chevron.r)) $
...
Honda–Tokoro (1991) and Boudol (1992) independently observed that *output prefix* (the ability to write $overbar(x) chevron.l y chevron.r. P$ for any $P$) requires the sender to know when the message has been received, an inherently synchronous primitive. Restricting outputs to the form $overbar(x) chevron.l y chevron.r$ (with no continuation, i.e. continuation forced to $0$) yields the *asynchronous $pi$-calculus*:

$ P ::= 0 | overbar(x) chevron.l y chevron.r | x(y). P | P bar Q | (nu x) P | !P $
...
The *polyadic* $pi$-calculus admits tuples in communications: $overbar(x) chevron.l y_1, dots, y_n chevron.r. P$ and $x(y_1, dots, y_n). P$. Encoding into the monadic calculus uses fresh channels:

$ chevron.l chevron.l overbar(x) chevron.l y_1, y_2 chevron.r. P chevron.r chevron.r = (nu w)(overbar(x) chevron.l w chevron.r. overbar(w) chevron.l y_1 chevron.r. overbar(w) chevron.l y_2 chevron.r. chevron.l chevron.l P chevron.r chevron.r) $
...
$ "Phil"_i eq.delta "think"_i. overbar("pick")_i. overbar("pick")_(i+1). "eat"_i. overbar("put")_i. overbar("put")_(i+1). "Phil"_i $
