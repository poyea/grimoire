...
*See also:* _Query Processing_ (traversing postings at query time), _Ranking: Classical Models_ (statistics stored in the index), _Vector Search_ (the analogous structure for dense embeddings), and the _Databases_ volume's chapters on B-trees and LSM-trees (Lucene segments are an LSM variant).
