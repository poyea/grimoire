...
1. *Allocate locally*: first-touch policy, where the physical frame backing a virtual page is allocated on the node of the touching CPU.
...
- *Memory leaks vs growth*: a process that keeps allocating without freeing is a leak; one that holds an unbounded cache is *growth*; the OS cannot distinguish between them.
