...
*See also:* `operating-systems/processes-and-threads.typ`, `operating-systems/file-systems.typ`, `operating-systems/storage-stack.typ`, `linux-kernel/introduction.typ`, `linux-kernel/kernel-modules.typ`, `cpu-architecture/cpu-fundamentals.typ`.
...
*Socket activation* inverts startup: the init system creates and listens on a service's socket *before* the service starts, then launches the service (passing it the already-open fd) only when the first connection arrives. Benefits: services start lazily, boot parallelizes freely (clients can connect to a socket whose server is still starting — the kernel buffers), and dependency ordering between two socket-activated services largely disappears.
