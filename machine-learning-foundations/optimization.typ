...
*See also:* _Linear Algebra for ML_ (matrix decompositions for second-order methods), _Pretraining_ in the Large Language Models volume (large-scale optimizer choices), _ML Workload Optimization on GPUs_ (memory/throughput tradeoffs), _Loss Functions_ (objective definitions that optimizers minimise).
