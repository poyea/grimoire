...
*See also:* #xref("machine-learning-foundations", "linear-algebra-for-ml", label: "Linear Algebra for ML") (matrix decompositions for second-order methods), #xref("llm", "pretraining", label: "Pretraining") in the Large Language Models volume (large-scale optimizer choices), #xref("gpu-architecture", "ml-workloads", label: "ML Workload Optimization on GPUs") (memory/throughput tradeoffs), #xref("machine-learning-foundations", "loss-functions", label: "Loss Functions") (objective definitions that optimizers minimise).
