...
*See also:* _Optimization_ (Hessian structure, Newton's method), _Deep Learning Fundamentals_ (initialization, backprop), `gpu-architecture/ml-workloads.typ` (matmul throughput), `llm/transformer-architecture.typ` (attention as low-rank composition).
