...
A sender ramps up its sending rate; somewhere along the path a bottleneck queue fills, packets drop, RTT rises, and goodput collapses. The job of congestion control is to find — and keep the flow near — the *bandwidth-delay product* (BDP) of the path without persistent queueing.
...
Reno triggers fast retransmit on three duplicate ACKs and fast recovery to skip slow-start after a single loss. Suffers badly when multiple segments are lost in one window — it exits fast recovery prematurely.
...
*Key property:* growth is independent of RTT — much fairer than Reno across heterogeneous-RTT flows, and far faster on high-BDP paths (transatlantic 1 Gb/s with $"RTT"=100"ms"$ needs to reach cwnd $approx 12500$ MSS; Reno would take $\sim 1000$ RTTs to recover from one loss, $"CUBIC"$ does it in seconds).
