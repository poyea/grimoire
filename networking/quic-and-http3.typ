...
*See also:* _Transport Layer_ (for TCP background), _Congestion Control_ (for $"BBR"$ and $"CUBIC"$), _TLS_ (QUIC integrates TLS 1.3), _Kernel Bypass_ (most QUIC stacks live in user space).
...
The QUIC *Connection ID* (CID) identifies a connection independently of the 4-tuple. When a client's IP changes (laptop suspends, phone switches Wi-Fi to LTE), it continues sending packets with the same CID; the server validates the new path with `PATH_CHALLENGE` / `PATH_RESPONSE` and resumes the connection — no TCP-style reconnection or TLS rehandshake.
...
- *MTU and amplification.* During the handshake, servers may reply with at most 3$times$ the bytes the client sent — limits initial server flight and prevents reflection attacks. Initial packets are padded to ≥ 1200 B to validate path MTU.
