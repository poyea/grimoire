...
Cost: ~1-3 cycles per spin iteration, but every contended atomic exchange invalidates the cache line on the holder's core. Above ~4 contenders, an unqueued spinlock collapses to O(n²) coherence traffic; this is why qspinlock / MCS / CLH variants exist.
...
+ Hash the mutex address into a global `ParkingLot` table; push the thread onto an intrusive bucket queue (a userspace queue with the same data-structure idea as case 2, but the "tasks" are OS threads).
...
static int __mutex_lock_common(struct mutex *lock, unsigned state, ...)
{
    ...
    //          between our failed fast CAS and entering the slow path)
