...
So for a *perfect* device sampling exactly from the ideal distribution, heavy outputs occur with probability $approx 0.854$ — substantially above $1/2$ because the Porter-Thomas distribution is heavy-tailed: there are many rare high-probability bitstrings, and an ideal device will frequently land on them. A *uniformly random* device (one that ignores the circuit entirely and samples uniformly) achieves exactly $1/2$.
